...
def t_tick_skips_when_checkout_vanishes_mid_sleep():
    ...
    marketplace reclone's stale-dir cleanup mid-session. The tick detects
    the missing checkout up front and prints exactly one skip line,
    spawning no subprocess against the vanished checkout at all."""
    with tempfile.TemporaryDirectory() as d:
        tmp = Path(d)
        checkout = _make_checkout(tmp)
        home = tmp / "home"
        ...
        try:
            # Startup (CHECKOUT resolution, GC) happens before the loop's
            # first `sleep 1` -- this window lets that settle before the
            # reclone simulation removes the dir.
            time.sleep(0.3)
