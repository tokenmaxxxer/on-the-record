...
- DELEGATION IS THE DEFAULT (issue #699 R2). This applies whether or not
  ...
  and delegate it to the matching skill instead of deciding it inline
  ...
  - A judgment whose answer does NOT need to change the repo (design/
    ...
    <n>]` — the matching skill's skill loaded, judgment rendered, answer returned as
    ...
    run_in_background. When two skills should judge concurrently and
    argue it out instead of one skill judging alone, the same no-branch/
    no-PR contract has a concurrent-judgment variant: `python3
    ${CHECKOUT}/spawn.py panel <skill_a> <skill_b> "<question>" [--issue
    <n>]`.
...
- YOUR GOAL LOOP (issue #699 R3) — this is what delegation is FOR, not an
  ...
  consult and each artifact to a spawned session (issue → branch → PR, per
  ...
  tracing which judgments went to which skill and what each one
  ...
  - YOUR DEVIATION LOOP (issue #803) — nests inside this goal loop, not a
    ...
    judgment a skill would normally render, a risk that would recur beyond
    ...
    File case — draft the issue, `spawn.py --skills <skill> "<task>"
    --issue <n> --no-wait`, append a `filed` line to the same log (timestamp,
...
- DELEGATION-CITING APPROVE (issue #707, demoted from
  ...
  provenance — a skill-bound session never posts one, for any skill; that
...
- DELEGATED AUTO-JUDGMENT (issue #573, demoted from
  ...
  mechanically reversible (impact axis), AND a multi-skill panel of every
  skill with standing reaches unanimous `approve`/`reject`
  (panel-unanimous-support-v1) — any missing precondition escalates to
  the user; never a solo skill's verdict, never an inline orchestrator
  call.
