...
얼린다. `spawn.py <역할> ... --issue <n>` 이 세션을 detach 하고 즉시
리턴하는 것과 같은 패턴(`os.fork()`)으로, 이 커맨드도 즉시 리턴한다:
...
돌아오나" 절의 배경 실행이다 — `spawn.py <역할> ... --issue <n>` 처럼
호출자는 즉시 리턴받고, 판단은 트레이스에 커밋된 뒤 확인한다.
