...
- `<역할>`: 판단을 물을 도메인을 자유 텍스트로 쓴다 — 고정된 역할 이름
  목록은 없다(이슈 #2610: 마지막 남은 닫힌 정체성 카탈로그가 은퇴했다).
  실제로 어떤 스킬 소스가 있는지는 인자 없이 `spawn.py` 를 부르면 뜨는
  skill-repository 체크아웃 디렉터리 목록에서 확인한다.
