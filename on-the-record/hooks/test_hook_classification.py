...
class HookClassificationTest(unittest.TestCase):
    ...
    def test_registration_count_matches_the_issues_own_count(self):
        ...
        # #3129 added amendment-channel.sh (wrapped) AND
        # amends-landing-apply.sh (wrapped, already live in hooks.json but
        # missing its own hook_classification.json entry until issue #3229
        # closed that gap), raising the real count to 16 (15 wrapped, 1
        # unwrapped) -- the "15 (14 wrapped)" this comment previously
        # recorded was stale from the moment amends-landing-apply.sh's
        # registration landed without a matching classification entry.
        # issue #3229 added delegation-live-check.sh (wrapped), raising it
        # to 17 (16 wrapped, 1 unwrapped) -- this literal is meant to move
        # again the next time a hook is legitimately registered; it exists
        # to catch drift, not to freeze the count at any one issue's number.
        live = registrations_from_hooks_json()
