...
#       ["cd", DIR, "&&", "python3"|"python", SPAWN_PATH, ...tail], with no
#       chaining/substitution operator token anywhere else in the list —
#       so this hook never blanket-allows an arbitrary command merely
#       because "spawn.py" appears in it somewhere, and an unbounded `cd`
#       directory slot can no longer swallow a command-substitution
#       payload before any operator check ever sees it.
...
# The whole command must tokenize (shlex.shlex(posix=True,
...
# ["python3"|"python", SPAWN_PATH, ...tail] or ["cd", DIR, "&&",
# "python3"|"python", SPAWN_PATH, ...tail], with no chaining/substitution
# operator token anywhere else in the list — closing the bypass where an
# unbounded `cd`-prefix directory slot swallowed a command-substitution
# payload before any operator search ever saw it (issue #834).
...
def _is_pybin(tok):
    return tok in ("python3", "python")


if len(tokens) >= 2 and _is_pybin(tokens[0]) and tokens[1].endswith("spawn.py"):
    spawn_path = tokens[1]
    tail = tokens[2:]
elif (len(tokens) >= 5 and tokens[0] == "cd" and tokens[2] == "&&"
      and _is_pybin(tokens[3]) and tokens[4].endswith("spawn.py")):
    spawn_path = tokens[4]
