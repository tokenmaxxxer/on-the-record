...
if [ -n "$_failed_open" ]; then
    case "$_hook_name" in
        session-role-bind.sh|directive.sh|post-landing-obligation-gate.sh|stop-gate.sh|skill-verdict-guard.sh|amends-landing-apply.sh|delegation-live-check.sh)
            _fallback_fired=1
