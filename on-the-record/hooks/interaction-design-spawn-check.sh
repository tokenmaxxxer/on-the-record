#!/usr/bin/env bash
# PreToolUse (Bash): issue #1130 routing-fix, cause-b role interaction-design.
# roles/specs/interaction-design.spec.json's use_when.trigger names
# record_absent_for: "interaction-design" — this hook is the missing
# consumer (docs/issue-1130/reports/requirements-engineering/scout-brief.md:
# the hook already works but never causes the role's own session to spawn).
#
...
# docs/issue-<n>/reports/interaction-design.md exists for the issue resolved
...
def deny(msg):
    sys.stderr.write("interaction-design-spawn-check: %s\n" % msg)
    sys.exit(2)
...
# parse (issue #1130 before-landing warrant hunt: `gh pr merge $(echo 1)`
# was silently let through by an earlier version of this check).
