...
def run_pair(pair_id: str, repo: str, skill_name: str, model: str,
    ...
    on_arm_manifest = [a for a in manifest["arms"] if a["arm"] == "on"][0]
    off_arm_manifest = [a for a in manifest["arms"] if a["arm"] == "off"][0]
    on_skills_root = on_arm_manifest["skills_root"]
    off_skills_root = off_arm_manifest["skills_root"]
