...
def run_pair(pair_id: str, repo: str, skill_name: str, model: str,
    ...
    unseeded = [a["arm"] for a in manifest["arms"]
                if not a.get("credentials_seeded", {}).get("seeded")]
    if unseeded:
        # Silent-failure-audit / issue #3245 independent-verification-1&2:
        # dispatching an arm whose isolated HOME has no CLI credentials
        # does not fail loudly -- `claude -p` exits nonzero on "Not
        # logged in" before any tool use, which `spawn.py doctor()`'s
        # `fired_ups and fired_pre` check misreports as "hooks silent in
        # headless mode," producing the exact misdiagnosis both
        # verifications traced (a fabricated CLI/hook regression, not a
        # real one). Failing closed here, with the actual reason
        # (`credentials_seeded`'s own recorded `reason`), keeps that
        # misdiagnosis from recurring instead of re-discovering it once
        # per run.
        prepare_arms._cleanup(created_dirs)
        reasons = {a["arm"]: a.get("credentials_seeded", {}).get("reason")
                   for a in manifest["arms"] if a["arm"] in unseeded}
        return {"pair_id": pair_id, "status": "credentials-not-seeded",
                "manifest": str(manifest_path), "transport": str(transport_path),
                "unseeded_arms": unseeded, "reasons": reasons,
                "excluded_from_h2": True,
                "exclusion_reason": f"credential seeding failed for arm(s) "
                                     f"{unseeded}: {reasons}",
                "h2": None}
