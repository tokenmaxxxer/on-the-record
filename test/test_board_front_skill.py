...
class FrontSkillTest(unittest.TestCase):
    ...
    def test_three_way_partial_tie_for_earliest_reports_cannot_decide(self):
        """Regression for the CHANGES review on PR #2735: a 2-candidate tie
        is not the only ambiguous shape. With 3+ rootless records, a
        *subset* can tie for the earliest introducing commit while a later,
        untied candidate also exists -- `len({sha for _, sha in
        candidates}) == 1` never fires here (the full set of shas is not
        identical), so before the fix the sort still returned an arbitrary
        winner among the tied pair with `ok=True`. `b` and `c` are added in
        the same commit (a genuine tie for earliest); `d` is added later in
        its own commit."""
        subject = "issue-9"
        _write_record(self.root, subject, "b")
        _write_record(self.root, subject, "c")
        _git(self.root, "add", f"docs/{subject}/reports/b.md", f"docs/{subject}/reports/c.md")
        _git(self.root, "commit", "-q", "-m", "b and c together")
        _write_record(self.root, subject, "d")
        _git(self.root, "add", f"docs/{subject}/reports/d.md")
        _git(self.root, "commit", "-q", "-m", "d later")
        front, ok = board._front_skill(self.root, subject, {"b": {}, "c": {}, "d": {}})
        self.assertEqual((front, ok), (None, False))
