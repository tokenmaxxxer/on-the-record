...
Two further independent adversarial-review verification sessions of
that second cut (docs/issue-2705/reports/adversarial-review-17a16473.md
and adversarial-review-f4b31b03.md) then live-reproduced a third layer
of bypass in the cwd-stack machinery itself, the fix's own new attack
surface: `BundledCwdStackFrameTest` pins `cd -` (the stack tracked no
OLDPWD-equivalent to restore to, so the generic `-`-prefix arg filter
silently swallowed the lone `-` token and left the stack unchanged),
`pushd`/`popd` (`popd` was not recognized as a directory-changing
keyword at all, and `pushd` reused the single-slot `cd` mutation
instead of actually pushing), and `cd` through a symlinked directory
component (the stack stored the lexical path, never `realpath`'d, so
the resolved repo-relative path did not match what `git status`
reports for the same real file).
...
class BundledCwdStackFrameTest(_GuardTestBase):
    """PR #2763's own cwd-stack machinery (fixing the cd/subshell,
    directory-add, and `:(exclude)` gaps above) modeled only a single
    mutable "current cwd" string. Two independent adversarial-review
    verification sessions ground-truthed the same three bypasses live
    in that stack: `cd -` (no OLDPWD-equivalent tracked to restore to,
    so the generic `-`-prefix arg filter silently dropped the lone `-`
    token), `pushd`/`popd` (`popd` unrecognized anywhere, `pushd`
    reusing the single-slot `cd` mutation instead of pushing), and `cd`
    through a symlinked directory component (the stack stored the
    lexical path, never canonicalized, so the resolved repo-relative
    path matched nothing in `git status`'s own real-path output)."""

    def test_cd_dash_restores_oldpwd_and_refuses_unregistered_gate(self):
        self._write("gates/new_gate_2705_cddash.py", "def check():\n    pass\n")
        self._write("sub/.keep", "")
        r = _run_guard(
            "cd sub && cd - && git add gates/new_gate_2705_cddash.py "
            "&& git commit -m x",
            str(self.repo))
        self.assertEqual(r.returncode, 2, r.stderr)

    def test_pushd_popd_restores_directory_and_refuses_unregistered_gate(self):
        self._write("gates/new_gate_2705_pushd.py", "def check():\n    pass\n")
        self._write("sub/.keep", "")
        r = _run_guard(
            "pushd sub && popd && git add gates/new_gate_2705_pushd.py "
            "&& git commit -m x",
            str(self.repo))
        self.assertEqual(r.returncode, 2, r.stderr)

    def test_cd_through_symlinked_directory_refuses_unregistered_gate(self):
        self._write("gates/new_gate_2705_symlink.py", "def check():\n    pass\n")
        (self.repo / "gates_link").symlink_to(self.repo / "gates")
        r = _run_guard(
            "cd gates_link && git add new_gate_2705_symlink.py "
            "&& git commit -m x",
            str(self.repo))
        self.assertEqual(r.returncode, 2, r.stderr)

    def test_popd_with_empty_stack_is_a_noop_not_a_crash(self):
        """`popd` with nothing pushed is a real-bash error ("directory
        stack empty") that leaves cwd unchanged -- the parser must not
        raise (e.g. IndexError) and must not treat it as a directory
        change."""
        self._write("gates/new_gate_2705_popd_empty.py", "def check():\n    pass\n")
        r = _run_guard(
            "popd && git add gates/new_gate_2705_popd_empty.py "
            "&& git commit -m x",
            str(self.repo))
        self.assertEqual(r.returncode, 2, r.stderr)
