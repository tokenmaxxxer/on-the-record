...
class JudgeModelGuardTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._tmpdir = tempfile.TemporaryDirectory()
        spawn.SKILL_MODEL_CONFIG = Path(self._tmpdir.name) / "role_model.txt"
        self._orig_resolve_consult_skill_source = spawn.resolve_consult_skill_source
        self._orig_core_plugin_dirs = spawn.core_plugin_dirs
        spawn.resolve_consult_skill_source = lambda skill, repo_root: {
            "source": "skill-repo", "skill_dirs": [Path("/fake/plugin")],
            "skills": ["fake"], "skill_sha": "abc1234", "unresolved": []}
    ...
    def tearDown(self):
        ...
        spawn.SKILL_MODEL_CONFIG = self._saved_config_path
        self._tmpdir.cleanup()
        spawn.resolve_consult_skill_source = self._orig_resolve_consult_skill_source
