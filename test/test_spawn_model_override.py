...
class ResolvedRoleModelPrecedenceTest(unittest.TestCase):
    """우선순위 레벨 하나당 테스트 하나: CLI > env > file > default.

    `spawn.ROLE_MODEL_CONFIG`는 레포 루트의 실제 `role_model.txt`를 가리키는
    ...
    def test_file_wins_over_default_when_no_cli_or_env(self):
        spawn.ROLE_MODEL_CONFIG.write_text("haiku")
        self.assertEqual(spawn.resolved_role_model(), "haiku")
...
class JudgeModelGuardTest(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        spawn.ROLE_MODEL_CONFIG = self._saved_config_path
        self._tmpdir.cleanup()
        spawn.plugin_dirs = self._orig_plugin_dirs
