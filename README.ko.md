...
별도 클론이나 시크릿 설정은 없다. 필요한 것은 `gh` 로그인뿐이다. 자세한
요구사항과 선택 설정(에이전트 계정, 모델 고정, 프로젝트 초기화)은
[`docs/handbooks/setup.md`](docs/handbooks/setup.md)에 있다.

## 상호작용 흐름

설치 이후의 메커니즘을 서술한다: 이슈, PR, 브랜치, record, 게이트.

### 사용자 쪽에서 보이는 루프

사용자는 대화로 요구를 말한다. 오케스트레이터가 이슈 초안을 써서 사용자
계정으로 GitHub에 릴레이하고, 사용자는 대화에서 확인한다. 이후 사용자의
입력과 AI의 활동은 각각 정해진 위치에 기록된다: 요구는 이슈, 결정은 승인
코멘트, 작업은 `issue-<n>/<role>` 브랜치의 PR, 근거는 그 PR의 record 문서.
역할은 phase 1(제안)을 먼저 내고, 사용자가 승인해야 phase 2(딜리버리)가
열린다. 사용자는 GitHub에 직접 쓰지 않는다. 모든 결정은 대화에서
오케스트레이터와 이루어지고, 오케스트레이터가 사용자 계정으로 GitHub에
릴레이한다: 코멘트로 피드백을, `APPROVE issue-<n>/<role>` 코멘트로 승인을,
머지로 수용을, 클로즈로 거절을. 오케스트레이터는 역할이 여는 각 PR도
감시하다가, 사용자가 결정하기 전에 그 내용을 대화로 보고·설명한다.
...
sequenceDiagram
    ...
    U->>O: 대화에서 피드백 또는 승인
    O->>G: 릴레이: 코멘트 = 피드백, 또는
    O->>G: 릴레이: "APPROVE issue-<n>/<role>" 코멘트 = 승인
    G-->>R: 승인 확인
    R->>G: phase 2 딜리버리 (같은 PR, record 문서 포함)
    G-->>O: watch가 딜리버리 감지
    O->>U: 대화로 딜리버리 내용 보고·설명
    U->>O: 대화에서 수용 또는 거절
    O->>G: 릴레이: 머지 = 수용, 또는 클로즈 = 거절
```
