...
def flows_payload(root: Path) -> dict:
    ...
    def comments_for(subject: str, pr_number: int) -> list[dict]:
        ...
        if issue_n not in comments_cache:
            comments_cache[issue_n] = _issue_comments(root, issue_n)
        out = list(comments_cache[issue_n])
        if pr_number and pr_number not in comments_cache:
            comments_cache[pr_number] = _issue_comments(root, pr_number)
        if pr_number:
    ...
    for subject, roles in sorted(b.items()):
        ...
        for role, fm in roles.items():
            ...
            if _front_role(root, subject, roles) == role:
...
def main() -> int:
    ...
    if a.role == "flows":
        # 상황판(repo-status-board)을 위한 읽기 전용 데이터 계약 (이슈 #172).
        # status()/closure-sweep 과 같은 줄: 아무것도 안 고치고, 안 posting 한다.
        return flows(a.cwd, a.json)
