...
|---|---|---|
| Batch 1 (this PR, issue #586 step 1) | Schema: assign `alignment` -> conformance-review, `external_burden` -> capacity-planning, `performance` -> performance-engineering on `roles/*.json`; extend `check_axis_ownership` to flag zero-owner axes **and wire it into `gates/role_spec_shape.py`'s `main()`/CLI entrypoint so it actually runs** (after-proposal hunt found the existing `check_axis_ownership`/`check_role_judgment_axes`/`check_axis_evaluation_entry` are dead code today — defined, unit-tested, never called from any production entrypoint, hook, or CI job; see `docs/reports/2026-08-10-hunt-issue-586-judgment-axis-matrix.md`); add the procedure-template section to the handbook | `roles/conformance-review.json`, `roles/capacity-planning.json`, `roles/performance-engineering.json`, `gates/role_spec_shape.py`, `gates/test_role_spec_shape_batch9.py`, `docs/handbooks/architecture-methodology.md`, `docs/decisions/2026-08-10-judgment-axis-matrix.md` |
