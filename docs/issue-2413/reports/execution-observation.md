...
upstream:
  - path: docs/issue-2413/reports/implementation.md (untracked on this
      ...
      via `git show 335a0c8e:docs/issue-2413/reports/implementation.md`)
...
subject: PR #2418 (issue-2413/implementation, still OPEN at observation
  time, HEAD f98a4af895a0b336b461fac4003792cafc5efa11) — its five
  acceptance-criteria claims about `_prune_spawn_attempts()`'s
  pid-liveness-plus-retention-age bound and `spawn_attempt_sweep()`'s
  per-tick subject dedup
test: independent re-derivation from source in a separate `git worktree`
  checked out at the PR's HEAD (not trusting the PR's own transcripts) —
  ran the PR's own pytest suite, then built an independently-authored
  422-line synthetic `spawn-attempts.jsonl` (not reusing the PR's own
  fixture) and ran the actual pre-fix (commit `2ca4b4de`) and post-fix
  `_prune_spawn_attempts()`/`spawn_attempt_sweep()` code against it
  directly, plus a real backgrounded `sleep 120` process for the
  live-in-flight claim, plus a targeted probe of the `ts`-fallback branch
result: failed
...
1. `_prune_spawn_attempts()`'s outcome-is-`None` branch never ages out a
   ...
   path: a small follow-up to `spawn.py:1064` — treat a missing `ts` the
   same as a non-numeric one (`aged_out = True` when the key is absent
   or not a number, instead of defaulting the value itself to `now`),
   plus a test case alongside the existing `SpawnAttemptPruneLiveness`
   class. Narrow in practice (the normal writer always sets `ts`; this
   requires the same class of hand-repair/corruption event the
   `pid`-string bug required), but real and unaddressed — the human's
   call whether it blocks landing PR #2418 or is filed as a fast
   follow-up.
...
records). Nothing further is queued in this session: the five
acceptance-claim checks, the test-suite runs, and the `ts`-fallback
reproduction quoted above under "What was done" cover the full scope
this record set out to check. Whether to require the `ts`-fallback fix
(open finding 1, resolution path stated there) before merging PR #2418,
land it as-is and file a fast follow-up, or accept the risk as out of
scope is the human's call, not this observation's to decide.
