...
skill-verdict: implementation-blueprint — applied: invoked; used to weigh the two-tier wiring placement (PreToolUse-blocking vs. gates/ci.py-advisory) before freezing which files each tier touches, given the work spans on-the-record/hooks/pretooluse_dispatcher.py, gates/gates.py, and gates/ci.py.
skill-verdict: silent-failure-audit — applied: invoked; used to characterize the two round-1 reliability defects (null byte, permission-denied directory) as the exact silent-failure shape (an observation failure reported as if it were a genuine empty/clean result) issue #3228 exists to catch, and to verify both fixes report rather than absorb.
skill-verdict: test-derivation — applied: invoked; used to derive the test partitions for silent-failure-lint-guard.sh (allow/deny boundary: with timeout, without timeout, allow-marker escape, non-.py, unrelated bare name, Edit fragment without enclosing def, MultiEdit with one bad edit among several, non-Write tool, kill switch, malformed payload) and for gates.silent_failure_new_findings (pre-existing-untouched vs. newly-added line, unparseable changed file, no-python-changes).
