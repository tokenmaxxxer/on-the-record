# Deviation — issue #2574 (execution-observation role)
...
denial, not a judgment call within this role's own authority to resolve.
...
approval-gate's closed-issue precondition denies unconditionally, before
any `CORE_BUILD_NOW`/APPROVE-comment path is evaluated, and
`CORE_BUILD_NOW` was confirmed absent from this session's own
environment (derived: `printenv | grep -i CORE_BUILD_NOW`, this session
— no output).
...
is reported here (not spawned) and in the session's own reply: the task
...
spawner-set `CORE_BUILD_NOW=1` stamp on a future respawn of this role, or
a human reopening issue #2574.

Note: an earlier, pre-sharding-convention copy of this same entry also
exists at
`docs/issue-2574/reports/execution-observation/deviation-log.md`
(committed this session at `a6f6974e`, following the flat-file pattern
`docs/issue-2180/reports/execution-observation/deviation-log.md` used);
this sharded file is the canonical location per `spawn.py
deviation-log-path --issue 2574`.
