...
canonical: docs/issue-866/reports/implementation/resolution.md, PR #875
(merged) — this record transcribes that write-up (produced during the
phase-1 session, when this path was mechanically approval-gated by
`on-the-record/hooks/approval-gate.sh`) into the standard phase-2 record
shape now that `APPROVE issue-866/implementation` has been posted. No
new claim is added beyond what `resolution.md` already states.
...
3. Reproduced — live, not by static reading — why
   ...
   `docs/issue-866/reports/implementation/survey.md`.
4. The after-proposal warrant hunt (stance 0, "assume the gate just
   touched is bypassable") surfaced a second, separate, and fixable gap
   in the same file: the hook's own trigger-detection regex, not its
   hash-comparison logic. See `## What did not work` below for the
...
6. The before-landing warrant hunt (stance 1, "assume this change and
   another plugin's rule cancel each other") surfaced that two sibling
   `PreToolUse`/`Bash` hooks — `gate-registration-guard.sh` and
   `role-axis-completeness-guard.sh` — carry the exact same pre-fix
   trigger regex `spec-index-preflight.sh` just moved away from, so the
   same `git -c <cfg>=<val> <verb> ...` shape still silently bypasses
   those two. Out of this issue's frozen write set; recorded as an open
   finding below, not fixed here.
...
- docs/issue-866/reports/implementation/2026-08-11-hunt-regenerate-spec-index-and-record-preflight-gap.md
  ...
  `## Open findings`)
- docs/issue-866/reports/implementation/resolution.md (phase-1 write-up
...
canonical: the phase-1 session's after-proposal `warrant:warrant-hunter`
dispatch (stance 0) and that session's own independent re-run of its
reproduction (transcript in Acceptance verification below, both carried
over from `docs/issue-866/reports/implementation/resolution.md`).

Actual: the hunter surfaced, and the phase-1 session verified live, that
...
done` step 3 (leave the hook and its test unmodified) and what the
phase-1 session actually left on the branch (the hook's trigger check
rewritten, 6 new test cases added).
...
returned one finding, verified independently by the phase-1 session
(below, not fixed — see `## Open findings`).

## Open findings

canonical: docs/issue-866/reports/implementation/2026-08-11-hunt-regenerate-spec-index-and-record-preflight-gap.md
(before-landing, stance 1), and the phase-1 session's own independent
check — `grep`-equivalent read of both files:
...
`docs/issue-866/`). Per the SCOPE-EXCEEDED rule, the phase-1 session
finished what the proposal covers and reported rather than widening the
write set mid-build to patch two files never proposed for this issue.
Needs a new issue: port the same `shlex.split`-based trigger fix to
...
own regression test mirroring the six cases added here (derived: `on-the-record/hooks/test_spec_index_preflight.py`, phase-1 session).
...
derived: `python3 -m pytest tests/test_spec_index.py -q`, phase-1
session, working tree with the index regen applied
...
canonical: the phase-1 session's own edit to
`docs/specs/reconciled-index.md`, cross-referenced against the fenced
transcript above.

derived: `python3 on-the-record/hooks/test_spec_index_preflight.py`,
phase-1 session, working tree with the hook fix and its 6 new
regression cases applied
...
phase-1 session, both resolving to
`103749e4de93d26ce061c88ada92f3edfa3a36b3` — the branch and `origin/main`
were the same commit prior to this change, so there was no
...
tests, and the docs/issue-866 files), phase-1 session:

```
1255 passed, 2 skipped, 1 xfailed in 199.29s (0:03:19)
```

`origin/main` (`103749e4de93d26ce061c88ada92f3edfa3a36b3`), phase-1
session:
...
canonical: `git log --oneline -3` and `git merge-base --is-ancestor HEAD origin/main`, this session, run against this branch after `git rebase origin/main`.
This branch's changes landed on `main` via PR #875 (squash-merged as
`7d97bd6`); this session's local commit that recorded the same content
(`ca74846`) was superseded by that squash and dropped by the rebase
before this record was added.

## Next steps

None — the code fix already landed on `main` (PR #875). This commit adds
the phase-2 record that PR's approval-gate blocked until
`APPROVE issue-866/implementation` was posted; once pushed and opened as
a PR with `Closes #866`, the issue closes on merge.
