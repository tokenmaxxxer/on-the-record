...
| `git-fetch-allow-gate.sh` | contract | new (issue #894, security-threat-model finding #1 fix): `PreToolUse`+`Bash`, grants `permissionDecision: "allow"` for a `git fetch [<remote>] [<refspec>...]` invocation (optionally `cd DIR &&`-prefixed), the one Bash shape a resumed orchestrator turn needed that `merge-allow-gate.sh`/`spawn-allow-gate.sh`/`gh-write-allow-gate.sh` didn't already cover, added so `--permission-mode bypassPermissions` could be dropped from `spawn.py::_resume_orchestrator_session`/`harness/driver.py::resume_orchestrator_session` without losing resume completion — scoped to the orchestrator session (`CLAUDE_ROLE` unset, same `session-role-bind.sh` snapshot primitive) with the same `shlex.shlex(posix=True, punctuation_chars=True)` strict shape validation as its siblings, no argument text past `fetch` ever inspected; only ever adds a permission signal, never denies; zero-install, ships with the plugin |
