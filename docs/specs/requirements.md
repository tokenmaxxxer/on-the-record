...
source_issue: 3041
check: scripts/issue-3041/run_pair.sh + evaluate_pair.py — NOT YET MEASURED. A first baseline run (PR #3052) reported 34 vs 33 with zero skill opens, and I recorded that here as a failing baseline. Independent verification (PR #3056) invalidated it: run_pair.sh passes --setting-sources project,local, which excludes the user scope where plugin/marketplace registration lives, so all 4 skills-on arms ran with plugins:[] and only the 17 built-in skills. The target skills were never in the corpus to select, making zero opens structurally guaranteed rather than a behavioural finding. The harness also ran against a target repo holding 3 scaffolding files and no application code. This requirement is unmeasured until a corrected run exists; issue #3053 carries the correction.
