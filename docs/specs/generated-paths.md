...
| `amendment-channel.sh` | n/a | the shell script itself makes no write call — it only execs `python3 amendment_channel.py`, which does the writing (`write_amendment`/`_write_seen`, both `$TMPDIR`-rooted: `${OTR_AMENDMENT_STATE_DIR:-$TMPDIR/otr-amendment}`, issue-keyed marker `issue-<n>.marker.json` plus per-(session, issue) seen files `seen/<session>__issue-<n>.json`, never inside the target repo, same out-of-tree pattern as `retry-loop-bound.sh`/`approach-cap-warning.sh`) |
