...
| `delegation-live-check.sh` | n/a | reads/checks only (issue #3229): reads `transcript_path` off the raw Stop event JSON, reads the `.on-the-record/delegation-state.json` state file via `delegation_state.load_state()`, and writes only to its own stdout/stderr — no write call inside this hook or `delegation_state.live_stop_decision()`/`grant()` (grant-writing is a separate, operator-initiated `spawn.py delegation-state --grant` CLI path, not something this Stop hook ever calls) |
...
| `amendment-channel.sh` | n/a | the shell script itself makes no write call — it only execs `python3 amendment_channel.py`, which does the writing (`write_amendment`/`_write_seen`, both `$TMPDIR`-rooted: `${OTR_AMENDMENT_STATE_DIR:-$TMPDIR/otr-amendment}`, repo-and-issue-keyed marker `issue-<n>__<repo>.marker.json` plus per-(session, repo, issue) seen files `seen/<session>__<repo>__issue-<n>.json` — the repo dimension (issue #3129 repair, cross-repo attribution leak) resolves locally via `repo_slug_for_cwd()` (`git remote get-url origin`, no network); an unresolvable repo skips the write/read entirely rather than falling into a shared bucket (issue #3128's shape), never inside the target repo, same out-of-tree pattern as `retry-loop-bound.sh`/`approach-cap-warning.sh`) |
...
| `skill-corpus-bootstrap.sh` | n/a | the shell script itself makes no write call — it only execs `python3 spawn.py ensure-skills` (`skills.py:ensure_skill_corpus_cli`), which does the writing: `Path.home() / ".claude" / "skills"` (out-of-tree, under the user's own home directory, never inside the target repo) and, via `_skill_repo_managed_root()`, `<checkout>/runs/rulebooks/skill-repository` (out-of-tree, the shared plugin checkout's own managed-clone cache, same location `self-update.sh`'s row above already covers for rulebooks/tokenmaxxxer-core) |
| `install-precondition-notices.sh` | n/a | reads/checks only (`git config --get`, a `docs/specs/approvers.md` existence test) — no write call anywhere in this hook (issue #3231) |
