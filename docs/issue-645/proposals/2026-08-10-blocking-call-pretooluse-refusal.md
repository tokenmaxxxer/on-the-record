...
4. Classify `command` against the enumerated blocking-shape pattern set
   (below). No match → exit 0 (allow, no cost imposed on ordinary
   commands).
5. Match found → check `run_in_background`. `true` → exit 0 (the
...
#### Blocking-shape pattern set (this issue's scoped enumeration)
Two shapes, matching the issue text plus the survey's confirmed
vocabulary (`--follow`, watchdog, poll-loop language already in run.md
rule 1):

- **`spawn.py watch` family**: command contains `spawn.py` followed by
  `watch` (with or without `--follow`), or `spawn.py watchdog` — the
  exact two calls run.md rule 1 already names as background-only.
  Refusal names the fix: `run_in_background: true` on the same call.
...
No third catch-all shape is added beyond these two — the issue names
these explicitly and step 1 found no attached transcript corpus beyond
the issue's own text (survey, "Blocking call shapes" section) to derive
further shapes from; a narrower, correctly-scoped enumeration is
preferred over a speculative broad one per this repo's stated
false-positive discipline (#600's precedent).
...
*does* close: the two literal shapes named by the issue, called by their
literal, unrenamed names — the observed failure mode, not a
hardened-against-adversarial-agent guarantee.
...
1 noting `blocking-call-guard.sh` now refuses the `spawn.py watch`/
watchdog and sleep-loop shapes pre-execution in the orchestrator session,
pointing at this file — matching how #600's rule 4 was cross-referenced
into the same section. No rule renumbering; this is a mechanism note, not
a new obligation (the obligation already exists as rule 1).
...
- **Red**: `spawn.py watch --follow` / `spawn.py watchdog` / a `while
  ...; do sleep 5; done` poll loop, each with `run_in_background` absent
  or `false`, in an orchestrator payload (`CLAUDE_ROLE` unset) →
  `{"decision":"block"}`, reason names the shape and the background fix.
- **Green — backgrounded**: identical commands with `run_in_background:
  true` → allow, unchanged.
- **Green — role session**: identical blocking commands with `CLAUDE_ROLE`
...
## Accumulation
One new hook file, one new test file, one array entry in `hooks.json`,
one cross-reference sentence in `run.md` — not a per-item inline
`subprocess`/`gh` call site, and not a `roles/*.json`-style repeated-file
edit. If a third blocking shape needs adding later, it extends the same
pattern-set list inside this one file rather than opening a new hook or
call site — matching #600's own accumulation note for the sibling check
this issue's design mirrors.
...
- Any change to `decision-queue-stopgate.sh` or #600's Stop-surface
  ...
  #600's file.
- Detecting blocking shapes beyond the two named by the issue text and
  confirmed by this survey (`spawn.py watch`/watchdog, sleep/poll loops)
  — no transcript evidence beyond the issue's own prose was available to
  step 1 to derive further shapes; a narrower correct enumeration is
  chosen over a speculative broader one.
...
[Claude Code harness] --PreToolUse stdin payload--> [blocking-call-guard.sh]
  ...
  gate 3: shape match on `command`?  -> no match: exit 0
  gate 4: run_in_background == true? -> yes: exit 0 (compliant background call)
...
- `python3 -m pytest on-the-record/hooks/test_blocking_call_guard.py -q`
  exits 0, including every red/green/fail-open case above.
...
- Manual replay: an orchestrator-session payload for `spawn.py watch
  --follow` with `run_in_background` absent is refused with a message
  naming `run_in_background: true` as the fix; the identical payload with
  `run_in_background: true` passes; the identical payload with
  `CLAUDE_ROLE` set passes.
