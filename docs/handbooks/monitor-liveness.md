...
Monitor's death reaches the orchestrator. See "Structural limit" below:
during a genuinely healthy, quiet stretch, this check's own invocation
is not bounded by anything in this repo. `poll_heartbeat_delta.py`'s
separate 1800s-bound beacon (issue #2915 round 2) does not close that
gap either — a dead tick loop cannot emit the beacon that would
announce its own absence. See "Issue #2915" under "Structural limit"
