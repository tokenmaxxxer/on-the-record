...
`PANEL_TIMEOUT`, `_VERB_REQUIRED_KEY`, `_VERB_INSTRUCTIONS`,
`_VERB_JSON_SHAPE`, `_JUDGE_EXCLUDED_CORE_PLUGINS`) — spawn.py re-exports
them by assignment.
...
# 위임을 모델에게 지시한다. skill_judge 자문 세션은 후보 스킬 중 관련
# 있는 것만 고르면 끝이라 이런 훅이 꽂히면 안 된다(이슈 #2201). core 의
