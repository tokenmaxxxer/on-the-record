#!/usr/bin/env python3
"""issue #525 (follow-up E of #515, batch-7) — validate the 4 commercial/risk-family named-lineage sub-batch (MEDDPICC/Dunford/ISO-44001/NIST-800-161)
...
structure with its own BATCH7_ROLES tuple (proposal constraint: don't edit
...
def test_batch7_spec_files_exist():
    for role in BATCH7_ROLES:
        assert (SPECS_DIR / f"{role}.spec.json").is_file(), f"missing spec: {role}"


def test_batch7_specs_pass_shape_check():
    bad = []
    for role in BATCH7_ROLES:
...
def test_batch7_specs_role_field_matches_filename():
    for role in BATCH7_ROLES:
        path = SPECS_DIR / f"{role}.spec.json"
        spec = json.loads(path.read_text(encoding="utf-8"))
