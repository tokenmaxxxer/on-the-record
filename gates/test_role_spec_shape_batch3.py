#!/usr/bin/env python3
"""issue #525 (follow-up E of #515, batch-3) — validate the 4 build-family named-lineage sub-batch (Spectral/MADR/dbt-contract; data-modeling per this sub-batch's own re-scout)
...
structure with its own BATCH3_ROLES tuple (proposal constraint: don't edit
...
def test_batch3_spec_files_exist():
    for role in BATCH3_ROLES:
        assert (SPECS_DIR / f"{role}.spec.json").is_file(), f"missing spec: {role}"


def test_batch3_specs_pass_shape_check():
    bad = []
    for role in BATCH3_ROLES:
...
def test_batch3_specs_role_field_matches_filename():
    for role in BATCH3_ROLES:
        path = SPECS_DIR / f"{role}.spec.json"
        spec = json.loads(path.read_text(encoding="utf-8"))
