#!/usr/bin/env python3
"""issue #525 (follow-up E of #515, batch-6a) — validate the 4 ops/knowledge-family second sub-batch
...
structure with its own BATCH6A_ROLES tuple (proposal constraint: don't edit
...
def test_batch6a_spec_files_exist():
    for role in BATCH6A_ROLES:
        assert (SPECS_DIR / f"{role}.spec.json").is_file(), f"missing spec: {role}"


def test_batch6a_specs_pass_shape_check():
    bad = []
    for role in BATCH6A_ROLES:
...
def test_batch6a_specs_role_field_matches_filename():
    for role in BATCH6A_ROLES:
        path = SPECS_DIR / f"{role}.spec.json"
        spec = json.loads(path.read_text(encoding="utf-8"))
