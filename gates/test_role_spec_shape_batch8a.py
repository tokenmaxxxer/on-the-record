#!/usr/bin/env python3
"""issue #525 (follow-up E of #515, batch-8a) — validate the 3 commercial/risk-family second sub-batch
...
structure with its own BATCH8A_ROLES tuple (proposal constraint: don't edit
...
def test_batch8a_spec_files_exist():
    for role in BATCH8A_ROLES:
        assert (SPECS_DIR / f"{role}.spec.json").is_file(), f"missing spec: {role}"


def test_batch8a_specs_pass_shape_check():
    bad = []
    for role in BATCH8A_ROLES:
...
def test_batch8a_specs_role_field_matches_filename():
    for role in BATCH8A_ROLES:
        path = SPECS_DIR / f"{role}.spec.json"
        spec = json.loads(path.read_text(encoding="utf-8"))
