...
def fetch_open_prs(repo: Path, run: Callable | None = None,
    ...
    quota, `gh api .../pulls` does not). Uses a `gh api -i` +
    `If-None-Match` + 304 pattern: an unchanged poll costs one REST
    call and bills no fresh data transfer, reusing the cached list on
    a 304. Returns
