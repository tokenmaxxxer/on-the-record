...
def skill_settings(skill: str, cwd: str | None = None,
    ...
    **병합**이라 필요하다 — 안 끄면 qa 스킬-저장소 가이던스만 적은 세션에 전역 17개가 딸려
    ...
    # 시점에 정해지는 슬러그다. 이슈 #2610: 그 슬러그를 조회하던 44개
    # role 카탈로그(파일 자체는 삭제됐다) — 어차피 이 함수는
    # `.get(role, {})`로 미조회 슬러그를 이미 빈 베이스라인으로 떨어뜨리고
    # 있었고(현재 신원 축의 모든 세션이 실제로 겪는 경로), 샌드박스는
    # 아래에서 역할과 무관하게 중앙에서 강제로 꺼지며(이슈 #695), 전역
    # 플러그인 차단·permissions.allow 보강도 spec 내용과 무관하게 항상
    # 적용된다 — 카탈로그가 실제로 값을 보태던 유일한 경로(레거시 44개
    # 이름의 env/sandbox 선언)는 그 이름들 자체가 현재 신원 축에서 쓰이지
    # 않아 이미 도달 불가능했다. 빈 베이스라인을 모두에게 무조건 적용해도
    # 관측 가능한 동작은 바뀌지 않는다.
    spec = {}
...
def spawn_cmd(settings_path: str, skill: str, unattended: bool,
    ...
    for p in (skill_dirs or []):
        cmd += ["--plugin-dir", str(p)]
    # MUSTER_SKILL_MODEL / role_model.txt (이슈#93): 스폰된 세션이 쓰는 모델을
...
def _admission_check_directive_completeness(ctx: dict) -> bool | None:
    ...
    try:
        ...
        # 이슈 #2507: 스폰 마운트 경로가 더 이상 role->skill 표를 안 쓴다
        # (`resolve_static_policy_source()` + 과제-텍스트 cross-family 매치로
        # 옮겼다, PR #2532) — 이 preflight 는 실제로 마운트될 것을 검증해야
        # 하므로 같이 옮긴다. cross-family 매치 자체(비동기, skill_judge
        # 자문 하나 필요)는 admission 시점에 미리 알 수 없어 여기서 트리거
        # 라인을 미리 검증할 수 없다 — 그 매치는 실제 마운트 시점에도
        # 실패하면 BM25 fail-open 하는 경로다
        # (`_cross_family_skill_matches_with_consult()` 독스트링), 그러니
        # admission 을 그 경로 때문에 막을 이유가 없다. 이슈 #2561: role->skill
        # 표(`_ROLE_SKILLS`)와 `resolve_role_source()` 는 마지막 소비부였던
        # `consult.py` 에서도 은퇴했다 — 스폰 마운트는 그대로
