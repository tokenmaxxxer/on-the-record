...
pipeline.py): every cross-function reference here resolves at call time
through `_sp` — the spawn module object, injected by spawn.py right after
it imports this module (guarded so only the canonical spawn/__main__ module
binds it), so `mock.patch.object(spawn, "<name>")` patches stay visible to
the moved code. This also covers `_sp.__file__`/`_sp.ROOT`: the checkpoint
blocks below embed spawn.py's own path in text handed to a spawned session
(`python3 <spawn.py path> await-approval ...`), so they must resolve
through the spawn module object, never `__file__` local to this file.
"""
